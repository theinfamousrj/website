$(function(){
	...
	/*  12. Calculations
	/*-------------------------------------------------------------------*/
	var calculateMilestones = function () {
		var coffees	= $('span.number[role="coffee-count"]')[0];
		var happys	= $('span.number[role="happy-count"]')[0];
		var hours	= $('span.number[role="hour-count"]')[0];
		var lines	= $('span.number[role="line-count"]')[0];

		var now = Date.now();

		var coffee	= Math.round((now - Date.parse('10/21/2001')) / (1000*60*60*24));
		var happy	= Math.round((now - Date.parse('08/16/2012')) / (1000*60*60*24));
		var hour	= Math.round(((now - Date.parse('08/16/2007'))*2080) / (1000*60*60*24*365.25));
		var line	= Math.round((now - Date.parse('08/16/2007')) / (1000*60*30));

		coffees.setAttribute('data-to', coffee);
		happys.setAttribute('data-to', happy);
		hours.setAttribute('data-to', hour);
		lines.setAttribute('data-to', line);
	};
